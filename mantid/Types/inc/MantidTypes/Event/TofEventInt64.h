...
class MANTID_TYPES_DLL TofEvent64 {
...
public:
  ...
  TofEvent64(double tof);

  /// Constructor, specifying the time of flight in microseconds and the frame
  /// id
  TofEvent64(double tof, const Core::DateAndTime &pulsetime);

  /// Empty constructor
  TofEvent64();

  bool operator==(const TofEvent64 &rhs) const;
   ...
   */
  bool operator<(const TofEvent64 &rhs) const { return (this->m_tof < rhs.m_tof); }
  /** < comparison operator, using the TOF to do the comparison.
  ...
  bool operator>(const TofEvent64 &rhs) const;
  bool equals(const TofEvent64 &rhs, const double tolTof, const int64_t tolPulse) const;
  ...
  friend std::ostream &operator<<(std::ostream &os, const TofEvent64 &event);
};
#pragma pack(pop)
 ...
 */
inline TofEvent64::TofEvent64(const double tof) : m_tof(tof), m_pulsetime(0) {}

/** Constructor, specifying the time of flight and the frame id
 * @param tof :: time of flight, in microseconds
 * @param pulsetime :: absolute pulse time of the neutron.
 */
inline TofEvent64::TofEvent64(const double tof, const Core::DateAndTime &pulsetime) : m_tof(tof), m_pulsetime(pulsetime.nanoseconds()) {}

/// Empty constructor
inline TofEvent64::TofEvent64() : m_tof(0), m_pulsetime(0) {}
 ...
 */
inline double TofEvent64::operator()() const { return m_tof; }

/** @return The 'x value'. Despite the name, this can be in any unit in the
 * UnitFactory.
 *  If it is time-of-flight, it will be in microseconds.
 */
inline double TofEvent64::tof() const { return m_tof; }

/// Return the pulse time
inline const Core::DateAndTime &TofEvent64::pulseTime() const { return Core::DateAndTime(m_pulsetime); }

/// Return the weight of the event - exactly 1.0 always
inline double TofEvent64::weight() const { return 1.0; }

/// Return the error of the event - exactly 1.0 always
inline double TofEvent64::error() const { return 1.0; }

/// Return the errorSquared of the event - exactly 1.0 always
inline double TofEvent64::errorSquared() const { return 1.0; }
} // namespace Event
} // namespace Types
} // namespace Mantid
